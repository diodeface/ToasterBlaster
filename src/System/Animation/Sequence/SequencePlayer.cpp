...
OverlaySequence* SequencePlayer::getRandomTransition() {
    if((random() & 0xff) < rareTransitionChance) {
...
bool SequencePlayer::transitionSequence(Sequence* sequence) {
    // todo: clean up this mess? it's getting worse and worse with each little addition
    ...
    if (eyeBlink) {
        ...
        // always crossfade if eyes are closed
        if (eyeBlink->getState() == EYEBLINK_CLOSED) {
            transitionOverlay->setSequence(&Transitions::crossfade);
