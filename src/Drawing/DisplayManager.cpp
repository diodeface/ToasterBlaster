...
DisplayManager::DisplayManager(Matrix* matrix, HeadsUpDisplay* hud, bool enabled) {
    ...
    u8 brightness = EEPROM.read(Config::EEPROM::MATRIX_BRIGHTNESS);
    setMatrixBrightness(brightness > 15 ? 15 : brightness);
}
...
void DisplayManager::addMatrixBrightness(i8 value) {
    setMatrixBrightness(matrix->getBrightness() + value);
}
...
void DisplayManager::stop() {
    enabled = false;
    for(Display* d : displays) {
        for(u8 i = 0; i < d->getSize(); i++) {
            matrix->drawColumn(d->getPosition() + i, 0);
        }
