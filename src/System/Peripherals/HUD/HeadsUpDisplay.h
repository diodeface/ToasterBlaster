...
class HeadsUpDisplay : public ToasterComponent {
  protected:
    HUDBuffer drawBuffer, renderedBuffer;
  public:
    ...
    virtual void drawXBM(u8 x, u8 y, u8 w, u8 h, Bitmap* xbm) = 0;
    ...
    virtual void drawStrX2(u8 x, u8 y, bool bold, const char* s) = 0;
    ...
    virtual void drawLine(u8 x1, u8 y1, u8 x2, u8 y2) = 0;

    virtual void drawStatusText(const char* str) = 0;

    virtual void update() {}

    virtual void setDrawBuffer(HUDBuffer buffer) = 0;
    
    virtual void setRenderedBuffer(HUDBuffer buffer) = 0;
