...
const char *ble_gap_evt_str(u8 event)
{
...
const char *bt_gap_evt_str(u8 event)
{
...
#if CONFIG_BT_BLE_ENABLED
static void add_ble_scan_result(esp_bd_addr_t bda, esp_ble_addr_type_t addr_type, u16 appearance, u8 *name, u8 name_len, i32 rssi)
{
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    u8 uuid_len = 0;
    u8 *uuid_d = esp_ble_resolve_adv_data(scan_rst->ble_adv, ESP_BLE_AD_TYPE_16SRV_CMPL, &uuid_len);
    if (uuid_d != NULL && uuid_len) {
        uuid = uuid_d[0] + (uuid_d[1] << 8);
    }

    u8 appearance_len = 0;
    u8 *appearance_d = esp_ble_resolve_adv_data(scan_rst->ble_adv, ESP_BLE_AD_TYPE_APPEARANCE, &appearance_len);
    if (appearance_d != NULL && appearance_len) {
        appearance = appearance_d[0] + (appearance_d[1] << 8);
    }

    u8 adv_name_len = 0;
    u8 *adv_name = esp_ble_resolve_adv_data(scan_rst->ble_adv, ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);
...
static esp_err_t start_ble_scan(u32 seconds)
{
...
esp_err_t esp_hid_ble_gap_adv_init(u16 appearance, const char *device_name)
{

    esp_err_t ret;

    const u8 hidd_service_uuid128[] = {
    ...
    esp_ble_adv_data_t ble_adv_data = {
        ...
        .p_service_uuid = (u8 *)hidd_service_uuid128,
        .flag = 0x6,
    };
    ...
    u8 init_key = ESP_BLE_ENC_KEY_MASK | ESP_BLE_ID_KEY_MASK;
    u8 rsp_key = ESP_BLE_ENC_KEY_MASK | ESP_BLE_ID_KEY_MASK;
    u8 key_size = 16; //the key size should be 7~16 bytes
    u32 passkey = 1234;//ESP_IO_CAP_OUT
    ...
    if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_SET_STATIC_PASSKEY, &passkey, sizeof(u32))) != ESP_OK) {
...
static esp_err_t init_low_level(u8 mode)
{
    esp_err_t ret;
...
esp_err_t esp_hid_gap_init(u8 mode)
{
    esp_err_t ret;
...
esp_err_t esp_hid_scan(u32 seconds, size_t *num_results, esp_hid_scan_result_t **results)
{
